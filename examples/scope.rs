...
fn activate(app: &gtk::Application) {
    ...
    match Scope::new(client_state) {
        ...
        Err(e) => {
            let builder = gtk::Builder::from_string(ERROR_DIALOG_XML);
            let dialog = builder
                .object::<gtk::MessageDialog>("error-dialog")
                .unwrap();
            dialog.set_application(Some(app));
            dialog.set_text(Some(&e.to_string()));
...
const ERROR_DIALOG_XML: &str = r#"
<?xml version="1.0" encoding="UTF-8"?>
<interface>
  <requires lib="gtk" version="4.0"/>
  <object class="GtkMessageDialog" id="error-dialog">
    <child type="action">
      <object class="GtkButton" id="close_response">
        <property name="label" translatable="true">Close</property>
      </object>
    </child>
    <action-widgets>
      <action-widget response="close">close_response</action-widget>
    </action-widgets>
  </object>
</interface>
"#;
